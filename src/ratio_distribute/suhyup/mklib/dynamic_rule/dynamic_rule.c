...
static bool parsing_serialized_json_and_make_a_linkedlist(char *serialized_json_msg, linkedlist_t *test_ll);
...
int main(int ac, char **av)
{
	...
	bool rule_change_event = true;


	if( ac != 2 ) {
		printf("Usage: $ %s rule_filename <enter>\n", av[0]);
		exit(0);
	}
	...
	while(true) {
		...
		if( rule_change_event == true ) {
			...
			tf = Load_json_make_rule( 0x00, json_tci_rule_file, map_delimiter, json_tci_ll);
			CHECK(tf);
			printf("rule reloading suceess.\n");
			rule_change_event = false;
		}
#endif
		...
		parsing_serialized_json_and_make_a_linkedlist( serialized_json_msg, test_ll );
