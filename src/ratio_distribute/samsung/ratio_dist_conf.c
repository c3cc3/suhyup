...
bool LoadConf(char *filename, ratio_dist_conf_t **my_conf, char **ptr_errmsg)
{
	...
	if(get_config(_config, "EVENTLOG_USE_FLAG", buffer) == NULL) {
		sprintf(ErrMsg, "EVENTLOG_USE_FLAG is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	if( STRCCMP(buffer, "true") == 0 ) {
		c->eventlog_use_flag = true;
	}
	else {
		c->eventlog_use_flag = false;
	}
