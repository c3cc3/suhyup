...
bool LoadConf(char *filename, fq_mon_svr_conf_t **my_conf, char **ptr_errmsg)
{
	...
	if(get_config(_config, "DELAY_SEC_THRESHOLD", buffer) == NULL) {
		sprintf(ErrMsg, "DELAY_SEC_THRESHOLD is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->delay_sec_threshold = atoi(buffer);
