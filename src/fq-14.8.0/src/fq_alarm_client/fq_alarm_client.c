...
static bool enQ_alarm_msg(fq_logger_t *l, fqueue_obj_t *alarm_en_q_obj, char *json_template_pathfile, alarm_msg_items_t *alarm_msg_st);
...
int main( int ac, char **av)
{
	...
	rc = OpenFileQueue(l, 0, 0, 0, 0, my_conf->qpath, my_conf->qname, &alarm_en_q_obj);
	CHECK(rc == TRUE);
...
static bool enQ_alarm_msg(fq_logger_t *l, fqueue_obj_t *alarm_en_q_obj, char *json_template_pathfile, alarm_msg_items_t *alarm_msg_st)
{
	...
	cache_update(cache_short_for_gssi, "hostname", alarm_msg_st->hostname, 0);
	cache_update(cache_short_for_gssi, "qpath", alarm_msg_st->qpath, 0);
	cache_update(cache_short_for_gssi, "qname", alarm_msg_st->qname, 0);
	cache_update(cache_short_for_gssi, "q_status_msg", alarm_msg_st->q_status_msg, 0);

	char str_msglen[20];
	sprintf(str_msglen, "%ld", (long)strlen(alarm_msg_st->q_status_msg));
	cache_update(cache_short_for_gssi, "msglen", str_msglen, 0);

	fq_log(l, FQ_LOG_DEBUG, "req_dttm=[%s], hostname=[%s], qpath=[%s], qname=[%s], msg=[%s], msglen=[%d]",
			req_dttm, alarm_msg_st->hostname,  alarm_msg_st->qpath,  alarm_msg_st->qname, strlen(alarm_msg_st->q_status_msg));

	char *var_data="111|aaa";
	char dst[8192];
	int rc;
	int file_len;
	...
	fq_log(l, FQ_LOG_DEBUG, "gssi_proc() result. [%s], rc=[%d], len=[%d]\n", dst, rc, strlen(dst));

	 bool tf;
	tf = uchar_enQ(l, dst, strlen(dst), alarm_en_q_obj);
	CHECK(tf);
