...
static void *data_thread(void *arg)
{
	...
	if(strcmp(cmd, "get") == 0 ) { /* enQ through the network */
		...
		while(1) {
			...
			rc = FQ_get_json(l, tcpobj, skey, path, qname, &qdata, &qdata_len, &result);
			if( rc < 0 ) {
...
retry_en:
			...
			rc =  FQ_put_json(l, tcpobj, (unsigned char *)en_buf, send_len, skey, path, qname,  &result);
			if( rc < 0 ) {
...
{  
	...
	*/
	...
	rc = FQ_link_json(l, 100, ip1, ip2, port, &tcpobj, path, qname, &result, &skey );
	if( rc < 0 || result != 'Y') {
	...
	fq_log(l, FQ_LOG_DEBUG, "link. skey=[%s]", skey);
