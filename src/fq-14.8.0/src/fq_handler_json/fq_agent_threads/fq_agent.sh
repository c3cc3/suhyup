...
PROCESS=deQ_put_agent
APP_CONF=deQ_put_agent.conf
...
then
    ...
    echo "       start : START deQ_put_agent."
    echo "       bstart : background START deQ_put_agent."
    echo "       stop  : STOP deQ_put_agent."
    echo "       kill  : KILL deQ_put_agent immediately."
    exit
fi
