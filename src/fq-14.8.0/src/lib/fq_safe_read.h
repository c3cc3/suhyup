...
*/
#ifndef _fq_safe_read_h
#define _fq_safe_read_h
...
typedef struct _safe_read_file_obj_t safe_read_file_obj_t;
struct _safe_read_file_obj_t {
	fq_logger_t *l;
	bool	binary_flag;
	char *filename;
	FILE	*fp;
	...
	int (*on_read)(safe_read_file_obj_t *obj, char **dst);
	int (*on_read_b)(safe_read_file_obj_t *obj, unsigned char **dst, int* ptr_msglen);
	int (*on_delete_file)(safe_read_file_obj_t *obj);
	off_t (*on_get_offset)(safe_read_file_obj_t *obj);
	float (*on_progress)(safe_read_file_obj_t *obj);
};
