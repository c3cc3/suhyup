 ...
 */
typedef enum { LEFT, RIGHT } adjust_t;
typedef enum { IS_QUAD, IS_LONG, IS_SHORT, IS_INT } var_type_enum;
...
#if 0
	    /* original */ if (prefix_char != NUL && s != S_NULL && s != char_buf) {
#endif
	    if (prefix_char != NUL && s != char_buf ) {
				*--s = prefix_char;
				s_len++;
	    }
...
int ap_strncpy(char *dst, size_t bufsize,  char* src, char* srcfile, int srcline)
{
	...
	if( n > bufsize-1 ) {
		fprintf(stderr,"Leak memory allocation call: src[%s] line[%d]\n", srcfile, srcline);
		fprintf(stderr, "allocation memory: [%ld]\n", (long int)bufsize);
		fprintf(stderr, "need memory      : [%ld]\n", (long int)strlen(src));
		memcpy( dst, src, bufsize-1);
		dst[bufsize-1] = '\0';
	}
	else {
