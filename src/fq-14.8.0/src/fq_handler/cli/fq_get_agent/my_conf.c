...
bool LoadConf(char *filename, my_conf_t **my_conf, char **ptr_errmsg)
{
	...
	if(get_config(_config, "REMOTE_DE_Q_PATH", buffer) == NULL) {
		sprintf(ErrMsg, "REMOTE_DE_Q_PATH is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->remote_de_qpath = strdup(buffer);

	if(get_config(_config, "REMOTE_DE_Q_NAME", buffer) == NULL) {
		sprintf(ErrMsg, "REMOTE_DE_Q_NAME is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->remote_de_qname = strdup(buffer);
...
void FreeConf( my_conf_t *t)
{
	...
	SAFE_FREE( t->remote_de_qpath);
	SAFE_FREE( t->remote_de_qname);
	SAFE_FREE( t->en_qpath);
	SAFE_FREE( t->en_qname);
