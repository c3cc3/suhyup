...
sess_remove_thread(void *arg)
{
	...
	while( true ) {
		...
		for (i=1; i <= MAX_SESSION_LIMIT; i++) {
			// printf("here: [%d]-th\n", i);
			if( SS.GlbSm[i].sess ) {
				if ( (SS.GlbSm[i].sess->last_access_time + tp->sess_timeout_sec) <  time(0) ) {
					fq_log(l, FQ_LOG_INFO, "expired session: sid = [%d].", i);
					sess_remove(l, i, 0, 0, 0); 
				}
			}
		}
#endif
...
int main(int ac, char **av)
{
	...
	*/
    ...
    CHECK(tp);
	tp->sess_timeout_sec = my_conf->sess_timeout_sec;

	int         s;
	pthread_t   sess_remove_thread_id;
	s = pthread_create(&sess_remove_thread_id, NULL, &sess_remove_thread, tp );

	if( s != 0 ) {
		printf("pthread_create() error.\n");
		return(0);
	}
	printf("sess_remove_thread started.\n");
	sleep(1);
	...
	fq_log(l, FQ_LOG_EMERG, "fq_handler server (CLANG Co.version %s) start!!!.(listening ip=[%s] port=[%s]", 
		FQ_PROTOCOL_SVR_C_VERSION, my_conf->server_ip, my_conf->server_port);
