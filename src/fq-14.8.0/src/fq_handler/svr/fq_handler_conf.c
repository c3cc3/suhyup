...
bool LoadConf(char *filename, fq_handler_conf_t **my_conf, char **ptr_errmsg)
{
	...
	if(get_config(_config, "SESS_TIMEOUT_SEC", buffer) == NULL) {
		sprintf(ErrMsg, "SESS_TIMEOUT_SEC is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->sess_timeout_sec = atoi(c->server_port);
