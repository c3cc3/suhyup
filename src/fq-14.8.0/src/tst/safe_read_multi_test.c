...
int main(int ac, char **av)
{
	...
	if( ac != 3 ) {
		printf("Usage: $ %s [data filename] [process_unique_id] <enter>\n", av[1]);
		exit(0);
	}
	data_filename = av[1];

	rc = fq_open_file_logger(&l, "/tmp/read_log_multi.log", get_log_level("debug"));
	CHECK(rc==TRUE);

	rc = open_file_4_read_safe_multi(l, data_filename, av[2], false, &obj);
	CHECK(rc == TRUE);
