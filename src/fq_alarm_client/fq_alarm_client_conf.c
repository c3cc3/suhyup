...
bool LoadConf(char *filename, fq_alarm_client_conf_t **my_conf, char **ptr_errmsg)
{
	...
	if(get_config(_config, "ERR_QPATH", buffer) == NULL) {
		sprintf(ErrMsg, "ERR_QPATH is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->err_qpath = strdup(buffer);

	if(get_config(_config, "ERR_QNAME", buffer) == NULL) {
		sprintf(ErrMsg, "ERR_QNAME is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->err_qname = strdup(buffer);

	if(get_config(_config, "ERR_MSG", buffer) == NULL) {
		sprintf(ErrMsg, "ERR_MSG is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->err_msg = strdup(buffer);
...
void FreeConf( fq_alarm_client_conf_t *t)
{
	...
	SAFE_FREE( t->err_qpath);
	SAFE_FREE( t->err_qname);
	SAFE_FREE( t->err_msg);
