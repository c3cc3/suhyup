...
static bool Open_Dist_Hist_Q( fq_logger_t *l, fq_alarm_svr_conf_t *my_conf, fqueue_obj_t **dist_hist_q_obj);
bool dist_history_enQ( fq_logger_t *l, char *channel, char co, fqueue_obj_t *dist_hist_q_obj, char *rcv_phone, char *send_phone_no, char *snd_msg);
void get_co_initial_from_qname( fq_logger_t *l, char *co_initial, fqueue_obj_t *enq_obj);
...
int main( int ac, char **av)
{
	...
	fqueue_obj_t *dist_hist_q_obj = NULL;
    tf = Open_Dist_Hist_Q( l, my_conf, &dist_hist_q_obj);
	if(tf == false) {
        fq_log(l, FQ_LOG_ERROR, "Can't open queue(path='%s', qname='%s'. Please check queue.", my_conf->dist_hist_qpath, my_conf->dist_hist_qname);
        return(0);
    }
	...
	while(1) {
		...
		if(rc < 0) { /* fatal error */
			...
			for( node_p=admin_phone_map_ll->head, t_no=0; (node_p != NULL); (node_p=node_p->next, t_no++) ) {
				...
				/* We send a distribute history to Gateway */

				char co_initial;
				get_co_initial_from_qname(l, &co_initial, enq_obj);

				tf = dist_history_enQ(l, "SS", co_initial, dist_hist_q_obj,  tmp->phone_no, my_conf->send_phone_no,  alarm_msg_st->msg );
				if(tf == false) {
					fq_log(l, FQ_LOG_ERROR, "dist_history_enQ('%s', '%s') failed.", dist_hist_q_obj->path, dist_hist_q_obj->qname);
				}
				fq_log(l, FQ_LOG_INFO, "We put a distribute info  to [%s][%s] successfully.", dist_hist_q_obj->path, dist_hist_q_obj->qname);
...
NEXT:
	...
	CloseFileQueue(l, &dist_hist_q_obj);
...
static bool Open_Dist_Hist_Q( fq_logger_t *l, fq_alarm_svr_conf_t *my_conf, fqueue_obj_t **dist_hist_q_obj)
{
	...
	rc =  OpenFileQueue(l, NULL, NULL, NULL, NULL, my_conf->dist_hist_qpath, my_conf->dist_hist_qname, &obj);
...
bool dist_history_enQ( fq_logger_t *l, char *channel, char co, fqueue_obj_t *dist_hist_q_obj, char *rcv_phone, char *send_phone_no, char *snd_msg)
{
	...
	json_object_set_string(json_root_object, "FAILURE_SENDER", send_phone_no);
	json_object_set_string(json_root_object, "FAILURE_RECEIVER", rcv_phone);
	json_object_set_string(json_root_object, "FAILURE_MESSAGE", snd_msg);
	...
	fq_log(l, FQ_LOG_INFO, "dist_result_enQ('%s', '%s') success. msg=[%s]", dist_hist_q_obj->path, dist_hist_q_obj->qname, buf);

	debug_json(l, json_root_value);
	int rc;

	rc = uchar_enQ(l, buf, buf_size, dist_hist_q_obj);
	CHECK( rc >= 0);
...
void get_co_initial_from_qname( fq_logger_t *l, char *co_initial, fqueue_obj_t *enq_obj)
{
	...
	p = strstr( enq_obj->qname, "KT");
	if( p ) {
		*co_initial = 'K';
	}
	else {
		p = strstr( enq_obj->qname, "LG");
		if( p ) {
			*co_initial = 'L';
		}
		else {
