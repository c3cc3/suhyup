...
bool LoadConf(char *filename, fq_alarm_svr_conf_t **my_conf, char **ptr_errmsg)
{
	...
	if(get_config(_config, "DIST_HIST_Q_PATH", buffer) == NULL) {
		sprintf(ErrMsg, "DIST_HIST_Q_PATH is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->dist_hist_qpath = strdup(buffer);

	if(get_config(_config, "DIST_HIST_Q_NAME", buffer) == NULL) {
		sprintf(ErrMsg, "DIST_HIST_Q_NAME is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->dist_hist_qname = strdup(buffer);
	...
	if(get_config(_config, "SEND_PHONE_NO", buffer) == NULL) {
		sprintf(ErrMsg, "SEND_PHONE_NO is undefined in your config file.");
		*ptr_errmsg = strdup(ErrMsg);
		return false;
	}
	c->send_phone_no = strdup(buffer);
